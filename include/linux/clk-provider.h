 ...
 *		__clk_init_parent.  This callback (or .get_parent_hw) is mandatory
 *		for clocks with
 *		multiple parents.  It is optional (and unnecessary) for clocks
 *		with 0 or 1 parents.
 *
 * @get_parent_hw: Queries the hardware to determine the parent of a clock.  The
 *		return value is a clk_hw pointer corresponding to the parent
 *		clock. In short, this function translates the parent value read
 *		from hardware into a pointer to the clk_hw for that clk.
 *		This callback (or .get_parent) is mandatory for clocks with
